...
index layouts. It is also not yet an end-to-end solve of unmodified
`InternalEMPIRE/empire.py` against the Julia program. The current Python
comparison is an independent Pyomo formulation of the same equations. A shared
interpretation error remains theoretically possible, although the separate
line-by-line source audit reduces that risk.
...
## Remaining bar for the strongest claim

Before claiming end-to-end equivalence with InternalEMPIRE rather than strong
formulation equivalence, run a controlled instance through the actual
`empire.py` model-construction code and compare its exported matrix/solution
against Julia. The reference needs a minimal, documented patch for its hidden
Hydrogen dependency if gas transport is included. The comparison should pin
all intentional differences above and use identical scenario tables, capacities,
solver tolerances, and result keys.
