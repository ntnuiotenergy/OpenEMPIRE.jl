...
# The natural-gas subsystem is compared against an independent Pyomo restatement
# of the same equations on a controlled three-node fixture. Hydrogen and Industry
# run their equivalent comparison from the suite; gas did not, and the script
# silently broke once as a result (it kept passing an `include_investment_constraints`
# keyword that no longer existed, while the suite stayed green). Wire it in so the
# same thing cannot happen again.
function test_natural_gas_controlled_solution_parity()
    python = get(
        ENV,
        "OPENEMPIRE_PYTHON",
    ...
    fixture = joinpath(root, "test", "data", "natural_gas_parity")
    julia_script = joinpath(root, "scripts", "natural_gas_parity_julia.jl")
    python_script = joinpath(root, "scripts", "natural_gas_parity_python.py")
    comparator = joinpath(root, "scripts", "compare_natural_gas_parity.py")
    mktempdir() do output_dir
        ...
        @test success(
            run(ignorestatus(`$python $comparator $julia_output $python_output`)),
        )
    end
end
