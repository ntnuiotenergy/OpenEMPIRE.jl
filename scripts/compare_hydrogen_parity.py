#!/usr/bin/env python3
"""Compare the controlled Julia and Pyomo Hydrogen/CO2 result tables."""
...
def read_results(path: Path) -> dict[Key, float]:
    ...
    for row in rows:
        ...
        if not math.isfinite(value):
            raise ValueError(f"{path}: non-finite result for {key}: {row['Value']!r}")
        result[key] = value
    if not result:
...
def compare(
    ...
    *,
    ...
    print(f"Compared {len(julia)} Hydrogen/CO2 metrics")
    ...
    print("Hydrogen/CO2 Julia/Pyomo parity: PASS")
    return len(julia), maximum_absolute, maximum_relative
