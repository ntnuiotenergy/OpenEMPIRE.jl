...
isdefined(@__MODULE__, :OpenEMPIREResults) ||
...
Base.@kwdef struct JuliaRunSpec{O, A <: Tuple, C}
    ...
    perf_enabled::Bool
    # Result dashboard, written after the solution CSVs. `plotly_js` vendors the
    # library next to the pages so an archived run renders without network access.
    plots::Bool
    plotly_js::String
end
...
function _resolve_run_spec(options)
    ...
    return JuliaRunSpec(;
        ...
        plots = _boolean_option(options["plots"], "plots"),
        plotly_js = options["plotly-js"],
    )
end
...
function _solve_model!(
    ...
    if JuMP.is_solved_and_feasible(emp)
        ...
        if spec.plots
            ...
            # A failure here must not lose a solved run: the CSVs are already on
            # disk and the dashboard can be regenerated with
            # scripts/plot_results.jl.
            try
                plotly_js = isempty(spec.plotly_js) ? nothing : spec.plotly_js
                dashboard_path = OpenEMPIREResults.write_result_plots(
                    spec.result_dir;
                    output_dir,
                    input_dir = spec.data_folder,
                    plotly_js,
                )
