...
function _resolve_run_spec(options)
    ...
    mkpath(result_dir)
    data_folder, config_file, fixed_investment_dir = _stage_run_inputs(
        result_dir,
...
function _solve_model!(spec::JuliaRunSpec, manifest, perf_phases, emp, sets, params, periods, progress)
    solve_start = time()
