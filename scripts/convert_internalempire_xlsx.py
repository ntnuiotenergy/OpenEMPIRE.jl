...
# InternalEMPIRE declares lineEfficiency over every DirectionalLink with
# `default=0.97`. The workbook omits the offshore cells, so preserve the Pyomo
# semantics explicitly in the converted CSV instead of relying on OpenEMPIRE's
# ordinary lossless fallback.
...
def mirror_internalempire_ccs_cost_omission(out: Path) -> None:
    """Match InternalEMPIRE's currently commented-out CCS transport/storage cost."""
...
def write_readme(source: Path, extra_out: Path, dataset: str, periods: int) -> None:
    lines = [
        ...
        "  internal modules read (`AvailableBioEnergy`,",
