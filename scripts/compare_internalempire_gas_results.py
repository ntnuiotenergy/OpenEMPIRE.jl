#!/usr/bin/env python3
"""Compare InternalEMPIRE and OpenEMPIRE.jl gas results on stable quantities.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--internal-log",
        type=Path,
        help="InternalEMPIRE Gurobi log; requires --julia-log and enforces certification",
    )
    parser.add_argument(
        "--julia-log",
        type=Path,
        help="OpenEMPIRE.jl Gurobi/stdout log; requires --internal-log",
    )
    ...
    with path.open(encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            if match := _OPTIMAL_OBJECTIVE.search(line):
                ...
                # Preserve the returned objective for alignment diagnostics while
                # leaving `certified_optimal` false. A sub-optimal result must be
                # reported as a certificate failure, not rejected by the parser.
                final_objective = float(match.group(1))
                final_objective_rounding = 0.5 * displayed_quantum(match.group(1))
...
def internal_components(root: Path, objective: float) -> dict[str, float]:
    components = investment_components(root)
    components["generator_operation"] = generator_operation_component(
        root / "results_objective_components_operational_costs.csv"
    )
    reported = math.fsum(components.values())
    # InternalEMPIRE's cost reports omit load shedding and natural-gas terminal
    # imports. Preserve that omission explicitly rather than pretending to have a
    # finer decomposition than the reference actually writes.
    components["unreported_operation"] = objective - reported
    return components
...
def julia_components(summary: dict[str, str]) -> dict[str, float]:
    ...
    result["unreported_operation"] = math.fsum(
        result.get(name, 0.0)
        for name in (
            "load_shedding",
...
def compare_capacities(
    ...
    for spec in CAPACITY_SPECS:
        internal = read_capacity_table(
            internal_root / spec.internal_file,
            "\t",
            spec.key_columns,
            spec.value_column,
...
def main() -> int:
    args = parse_args()
    if (args.internal_log is None) != (args.julia_log is None):
        raise ValueError("--internal-log and --julia-log must be supplied together")
    ...
    print("InternalEMPIRE ↔ OpenEMPIRE.jl natural-gas verification")
    ...
    logs_ok = True
    bracket_ok = True
    if args.internal_log is not None and args.julia_log is not None:
    ...
    print("\nObjective components (InternalEMPIRE cost report vs Julia summary.txt):")
    internal_costs = internal_components(internal_root, internal_total)
    ...
    print(
        ...
        "(small residual is reference-report reconstruction precision)"
    )
    ...
    print("\nNatural-gas aggregate balance diagnostics (gross routing is non-unique):")
    print("\n".join(natural_gas_aggregate_diagnostics(internal_root, julia_output)))

    print("\nGenerator-operation objective differences by stable aggregate key:")
    print(
        "\n".join(
            report_generator_cost_differences(
                internal_root,
                julia_output,
                args.discount_rate,
                args.period_years,
    ...
    print(
        ...
        + f"logs_ok={logs_ok} bracket_ok={bracket_ok} capacities_ok={capacities_ok}"
    )
