...
#$ -cwd
#$ -V
#$ -o logs/julia_gasfull_$JOB_ID.out
#$ -e logs/julia_gasfull_$JOB_ID.err
...
CONFIG_FILE=config/run_int_full_gas.yaml
INPUT_FORMAT=csv
JULIA_CMD=${JULIA_CMD:-julia}
JULIA_SOLVER=${JULIA_SOLVER:-Gurobi}
JULIA_SEED=${JULIA_SEED:-1}
RESULTS_DIR=${RESULTS_DIR:-/storage/users/torgrif/int_full_gas_jl}
...
if [ -z "$JOB_ID" ]; then
	...
	qsub -l hostname=${BEST_NODE} \
		...
		"$0"
	echo "Submitted. Monitor with qstat and logs/julia_gasfull_<JOB_ID>.out"
	exit 0
fi
