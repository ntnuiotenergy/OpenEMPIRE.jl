...
if [[ "$CLUSTER" == "Solstorm" ]]; then
	...
	ssh "$REMOTE_USER@$REMOTE_SERVER" \
		"cd $REMOTE_DIR && JULIA_SOLVER='$JULIA_SOLVER' JULIA_SEED='$JULIA_SEED' JULIA_OPTIMIZE='$JULIA_OPTIMIZE' JULIA_FIXED_SAMPLE='$JULIA_FIXED_SAMPLE' JULIA_CMD='$JULIA_CMD' EMPIRE_PERF='$EMPIRE_PERF' EMPIRE_PERF_INTERVAL='$EMPIRE_PERF_INTERVAL' JULIA_SGE_HOSTS='$JULIA_SGE_HOSTS' sh $SCHEDULER_SCRIPT"
else
