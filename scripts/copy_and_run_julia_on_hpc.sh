...
JULIA_SOLVER=${JULIA_SOLVER:-$(jq -r ".$CLUSTER.JULIA_SOLVER // \"HiGHS\"" "$CONFIG_FILE")}
JULIA_SEED=${JULIA_SEED:-$(jq -r ".$CLUSTER.JULIA_SEED // \"1\"" "$CONFIG_FILE")}
JULIA_OPTIMIZE=${JULIA_OPTIMIZE:-$(jq -r ".$CLUSTER.JULIA_OPTIMIZE // \"true\"" "$CONFIG_FILE")}
JULIA_FIXED_SAMPLE=${JULIA_FIXED_SAMPLE:-$(jq -r ".$CLUSTER.JULIA_FIXED_SAMPLE // \"false\"" "$CONFIG_FILE")}
JULIA_CMD=${JULIA_CMD:-$(jq -r ".$CLUSTER.JULIA_CMD // \"julia\"" "$CONFIG_FILE")}
...
if [[ "$CLUSTER" == "Solstorm" ]]; then
	...
	ssh "$REMOTE_USER@$REMOTE_SERVER" \
		"cd $REMOTE_DIR && JULIA_SOLVER='$JULIA_SOLVER' JULIA_SEED='$JULIA_SEED' JULIA_OPTIMIZE='$JULIA_OPTIMIZE' JULIA_FIXED_SAMPLE='$JULIA_FIXED_SAMPLE' JULIA_CMD='$JULIA_CMD' EMPIRE_PERF='$EMPIRE_PERF' EMPIRE_PERF_INTERVAL='$EMPIRE_PERF_INTERVAL' sh $SCHEDULER_SCRIPT"
else
